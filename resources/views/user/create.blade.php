<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-900 tracking-tight">Tambah User</h1>
                <p class="text-xs text-slate-500 mt-0.5">Tambahkan akun pengguna baru</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="content-section-header">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background: var(--gradient-primary);">
                        <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                    </div>
                ...
                <form method="POST" action="{{ route('user.store') }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div class="form-group sm:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" class="form-input" placeholder="Masukkan nama lengkap">
                            @error('name') <p class="text-rose-500 text-xs mt-1.5 font-medium">{{ $message }}</p> @enderror
                        </div>
                        
                        <div class="form-group sm:col-span-2">
                            <label>Email</label>
                            <input type="email" name="email" value="{{ old('email') }}" class="form-input" placeholder="Masukkan alamat email">
